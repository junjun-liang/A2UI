...
<div style="text-align: center;">
  <div class="centered-logo-text-group">
    ...
    <h1>A2UI (Agent to UI) 协议 v0.10</h1>
  </div>
</div>
...
A2UI 协议旨在通过从服务器（Agent）发送的 JSON 对象流来动态渲染用户界面。其核心理念强调 UI 结构与应用数据的清晰分离，支持客户端在处理每条消息时进行渐进式渲染。
...
有关 v0.9 和 v0.10 之间差异的详细说明，请参阅[演进指南](evolution_guide.md)。

## 协议概述与数据流

A2UI 协议使用从服务器到客户端的 JSON 消息单向流来描述和更新 UI。客户端消费此流，构建 UI 并渲染。用户交互则单独处理，通常通过向不同的端点发送事件来处理，这可能会在 UI 流上触发新消息。

以下是一个事件序列示例（不必完全按照此顺序）：

1.  **创建 Surface：** 服务器发送 `createSurface` 消息来初始化 Surface。
2.  **更新 Surface：** 一旦 Surface 被创建，服务器发送一个或多个 `updateComponents` 消息，包含将成为该 Surface 一部分的所有组件的定义。
3.  **更新数据模型：** 一旦 Surface 被创建，服务器可以随时发送 `updateDataModel` 消息来填充或更改 UI 组件将显示的数据。
4.  **渲染：** 客户端渲染该 Surface 的 UI，使用组件定义构建结构，使用数据模型填充内容。
5.  **动态更新：** 当用户与应用交互或有新信息可用时，服务器可以发送额外的 `updateComponents` 和 `updateDataModel` 消息来动态更改 UI。
6.  **删除 Surface：** 当 UI 区域不再需要时，服务器发送 `deleteSurface` 消息来移除它。
...
A2UI 协议被设计为与传输层无关。它定义了 JSON 消息结构以及服务器（Agent）和客户端（渲染器）之间的语义契约，但不强制要求特定的传输层。

### 传输契约

要支持 A2UI，传输层必须满足以下契约：

1.  **可靠交付：** 消息必须按照生成的顺序交付。A2UI 依赖有状态更新（例如，在更新之前先创建 Surface），因此乱序交付可能会损坏 UI 状态。
2.  **消息帧定界：** 传输层必须清晰地分隔各个 JSON 信封消息（例如，在 JSONL 中使用换行符、WebSocket 帧或 SSE 事件）。
3.  **元数据支持：** 传输层必须提供将元数据与消息关联的机制。这对于以下方面至关重要：
    - **数据模型同步：** `sendDataModel` 功能要求客户端将当前数据模型状态作为元数据与用户操作一起发送。
    - **能力交换：** 客户端能力（支持的目录、自定义组件）和服务器能力通过元数据或传输层特定的握手（如 A2A 中的 Agent Card 或 MCP 中的初始化）进行交换。
4.  **双向能力（可选）：** 虽然渲染流是单向的（服务器 -> 客户端），但交互式应用需要用于 `action` 消息（客户端 -> 服务器）的返回通道。

### 传输绑定

虽然 A2UI 与传输层无关，但它最常用于以下传输方式。

#### A2A (Agent2Agent) 绑定

[A2A (Agent-to-Agent)](https://a2a-protocol.org/latest/) 是代理系统中 A2UI 的优秀传输选择，通过额外的载荷扩展 A2A。
A2A 具有处理远程代理通信的独特能力，还可以在代理后端和前端应用之间提供安全高效的传输。

- **消息映射：** 每个 A2UI 信封（例如 `updateComponents`）对应单个 A2A 消息 Part 的载荷。
- **元数据**：
...
**[AG-UI](https://docs.ag-ui.com/introduction)** 也是 A2UI Agent-用户交互协议的优秀传输选择。
AG UI 提供与许多代理框架和前端的便捷集成。AG UI 提供前端和代理后端之间的低延迟和共享状态消息传递。
...
- **[SSE](https://en.wikipedia.org/wiki/Server-sent_events) 与 [JSON RPC](https://www.jsonrpc.org/)**：用于支持流式传输的 Web 集成的标准服务器发送事件，以及用于客户端-服务器通信的 JSON RPC。
- **[WebSockets](https://en.wikipedia.org/wiki/WebSocket)**：用于双向实时会话。
- **[REST](https://cloud.google.com/discover/what-is-rest-api?hl=en)**：对于简单用例，REST API 可行但缺乏流式传输能力。

## 协议模式

A2UI v0.10 由三个相互交互的 JSON 模式定义。
...
[`server_to_client.json`] 模式是顶层入口点。服务器流式传输的每条消息都必须根据此模式验证。它处理消息分发。
...
[`server_to_client.json`] 信封模式被设计为与目录无关。它使用占位符文件名引用组件和主题：`catalog.json`（具体为 `$ref: "catalog.json#/$defs/anyComponent"` 和 `$ref: "catalog.json#/$defs/theme"`）。
...
这种间接方式允许相同的核心信封模式与任何合规的组件目录一起使用，而无需修改。

自定义目录可用于定义额外的 UI 组件或修改现有组件的行为。要使用自定义目录，只需将其包含在提示中替代基础目录即可。它应具有与基础目录相同的形式，并使用 [`common_types.json`] 模式中的通用元素。

### 验证器合规性与自定义目录

为确保自动化验证器能够验证 UI 树的完整性（检查父组件是否引用了存在的子组件），自定义目录必须遵循以下严格的类型规则：
...
验证器通过查找这些特定的模式引用来确定哪些字段代表结构链接。如果对 ID 使用原始字符串类型，验证器会将其视为静态文本（如 URL 或标签），不会检查目标组件是否存在。

## 信封消息结构

信封定义了四种主要消息类型，服务器流式传输的每条消息必须是包含以下键之一的 JSON 对象：`createSurface`、`updateComponents`、`updateDataModel` 或 `deleteSurface`。该键指示消息类型，这些是构成协议流中每条消息的内容。

### `createSurface`

此消息通知客户端创建一个新的 Surface 并开始渲染。在向 Surface 发送任何 `updateComponents` 或 `updateDataModel` 消息之前，必须先创建该 Surface。虽然通常通过 Agent 发送 `createSurface` 消息来实现，但如果 Agent 知道 Surface 已被创建（例如由另一个 Agent 创建），则可以跳过此步骤。一旦 Surface 被创建，其 `surfaceId` 和 `catalogId` 即被固定；要重新配置它们，必须删除并重新创建 Surface。组件列表中必须有一个组件的 `id` 为 `root`，作为组件树的根节点。

**属性：**

- `surfaceId`（字符串，必需）：要渲染的 UI Surface 的唯一标识符。
- `catalogId`（字符串，必需）：唯一标识用于此 Surface 的目录（组件和函数）的字符串。建议使用您拥有的互联网域名作为前缀，以避免冲突（例如 `https://mycompany.com/1.0/somecatalog`）。如果是 URL，该 URL 不需要有任何部署的资源，它只是一个唯一标识符。
- `theme`（对象，可选）：包含目录主题模式中定义的主题参数（例如 `primaryColor`）的 JSON 对象。
- `sendDataModel`（布尔值，可选）：如果为 true，客户端将在发送到服务器的每条消息的元数据中（通过传输层的元数据机制）发送此 Surface 的完整数据模型。这确保 Surface 所有者与用户的操作或查询一起接收 UI 的完整当前状态。默认为 false。
...
此消息提供要添加到或更新特定 Surface 中的 UI 组件列表。组件以扁平列表形式提供，它们之间的关系通过邻接表中的 ID 引用定义。此消息只能发送到已创建的 Surface。请注意，组件可能引用尚不存在的子组件或数据绑定；客户端应通过渲染占位符（渐进式渲染）来优雅地处理此情况。

**属性：**

- `surfaceId`（字符串，必需）：要更新的 UI Surface 的唯一标识符。这通常是一个有意义的名称（例如 "user_profile_card"），并且在 GenUI 会话的上下文中必须是唯一的。
- `components`（数组，必需）：组件对象列表。组件以扁平列表形式提供，它们之间的关系通过邻接表中的 ID 引用定义。
...
此消息用于发送或更新填充 UI 组件的数据。它允许服务器更改 UI 的内容而无需重新发送整个组件结构。`updateDataModel` 消息将指定 `path` 处的值替换为新内容。如果省略 `path`（或为 `/`），则替换 Surface 的整个数据模型。
...
A2UI 的组件模型旨在提供灵活性，将协议结构与可用 UI 组件集分离。
...
A2UI 协议将 UI 定义为组件的扁平列表。树结构通过 ID 引用隐式构建。这被称为邻接表模型。

容器组件（如 `Row`、`Column`、`List` 和 `Card`）具有引用其子组件 `id` 的属性。客户端负责将所有组件存储在映射中（例如 `Map<String, Component>`），并在渲染时重建树结构。

此模型允许服务器以任意顺序发送组件定义。一旦定义了 `root` 组件，渲染即可开始，客户端会在额外的定义到达时渐进地填充或更新树的其余部分。

组件树中必须恰好有一个 ID 为 `root` 的组件，作为组件树的根。在该组件被定义之前，其他组件更新不会产生可见效果，它们将被缓冲直到定义了根组件。一旦定义了根组件，客户端负责根据可用数据以最佳方式渲染树，跳过无效引用。
...
交互式组件（如 `Button`）使用 `action` 属性来定义用户与之交互时发生的事情。操作可以触发发送到服务器的事件，也可以执行本地客户端函数。
...
- 以 `/` 开头的路径（例如 `/user/profile/name`）是**绝对路径**。它们始终从数据模型的根解析，无论组件在 UI 树中嵌套在哪里。
...
- **相对解析：** 在这些实例化的组件中，任何**不以**正斜杠 `/` 开头的路径都被视为**相对路径**。
  - 在遍历 `/users` 的模板中，相对路径 `firstName` 对第一项解析为 `/users/0/firstName`，对第二项解析为 `/users/1/firstName`，依此类推。

- **混合作用域：** 子作用域中的组件仍然可以通过使用绝对路径访问根作用域。
...
- **数字/布尔值**：标准字符串表示。
- **null/undefined**：空字符串 `""`。
- **对象/数组**：序列化为 JSON 以确保不同客户端实现之间的一致性。
...
1.  **读取（模型 -> 视图）：** 当组件渲染时，它从绑定的 `path` 读取其值。如果通过 `updateDataModel` 更新了数据模型，组件将重新渲染以反映新值。
2.  **写入（视图 -> 模型）：** 当用户与组件交互（例如，输入字符、切换复选框）时，客户端**立即**更新本地数据模型中绑定 `path` 处的值。
...
- 用户输入（按键、切换）**不会**自动触发向服务器的网络请求。
- 更新的状态仅在触发特定的**用户操作**（例如，点击 `Button`）时才发送到服务器。
- 当分发 `action` 时，action 的 `context` 属性可以引用修改过的数据路径，将用户的输入发送回服务器。

#### 示例：表单提交模式

1.  **绑定：** `TextField` 绑定到 `/formData/email`。
2.  **交互：** 用户输入 "jane@example.com"。本地模型中 `/formData/email` 被更新。
...
4.  **发送：** 点击时，客户端解析 `/formData/email`（得到 "jane@example.com"）并在 `action` 载荷中发送。

## 数据模型更新：同步与收敛

虽然上面的章节描述了组件如何引用数据，但本节定义了数据模型本身如何被**更新**和同步。

为了支持渲染器与创建 Surface 的 Agent 之间的可靠数据同步，A2UI 协议使用由 `createSurface` 消息中的 `sendDataModel` 属性控制的简单同步机制。
...
当 Surface 的 `sendDataModel` 设置为 `true` 时，客户端会自动将该 Surface 的**整个数据模型**附加到发送给创建该 Surface 的服务器的每条消息（如 `action` 或用户查询）的元数据中。数据模型使用传输层的元数据功能（例如，A2A 中的 `metadata` 字段或 HTTP 中的头部）包含。载荷遵循 [`client_data_model.json`](../json/client_data_model.json) 中的模式。

- **定向交付：** 数据模型仅发送给创建 Surface 的服务器。数据不会泄漏给其他 Agent 或服务器。
- **触发：** 数据仅在触发客户端到服务器消息时发送（例如，通过用户操作如按钮点击）。被动数据变化（如在文本字段中输入）本身不会触发网络请求；它们只是更新本地状态，将在下一次操作时发送。
- **载荷：** 数据模型包含在传输元数据中，由其 `surfaceId` 标记。
- **收敛：** 服务器将接收到的数据模型视为操作时客户端的当前状态。

## 客户端逻辑与验证

A2UI v0.10 将客户端逻辑泛化为**函数**。这些函数可用于验证、数据转换和动态属性绑定。

### 注册函数

客户端支持一组命名的**函数**（例如 `required`、`regex`、`email`、`add`、`concat`），这些函数在 JSON 模式（例如 `basic_catalog.json`）中与组件定义一起定义。服务器在 `FunctionCall` 对象中通过名称引用这些函数。这避免了发送可执行代码。
...
[`basic_catalog.json`] 提供了组件和函数的基线集合。
...
| **Divider**       | 水平或垂直分割线。                                                                       |
| **Modal**         | 由主内容中的按钮触发的、覆盖在主内容上的对话框。                                          |
| **Button**        | 可点击的按钮，分发操作。支持 'primary' 和 'borderless' 变体。                             |
| **CheckBox**      | 带标签和布尔值的复选框。                                                                  |
| **TextField**     | 用户文本输入字段。                                                                       |
...
| 属性                  | 类型   | 描述                                                                                                                                                                |
| :-------------------- | :----- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **primaryColor**      | String | 用于整个 UI 高亮显示的主要品牌颜色（例如，主按钮、活动边框）。渲染器可根据需要生成变体，如较浅的色调。格式：十六进制代码（例如 '#00BFFF'）。                          |
| **iconUrl**           | URI    | 用于标识与 Surface 关联的 Agent 或工具的图片（例如标志或头像）的 URL。                                                                                               |
...
在多 Agent 系统或编排器中，编排器负责设置或验证这些字段。这确保向用户显示的身份与实际联系的 Agent 服务器匹配，防止恶意 Agent 冒充可信服务。例如，编排器可能会在将 `createSurface` 消息转发给客户端之前，用子 Agent 的已验证身份覆盖这些字段。
...
- `${now()}`：无参数的函数。
- `${formatDate(value:${/currentDate}, format:'yyyy-MM-dd')}`：带命名参数的函数。

参数可以是**字面量**（带引号的字符串、数字或布尔值）或**嵌套表达式**。

#### `formatString` 嵌套插值

表达式可以通过在外部表达式内部使用额外的 `${...}` 包装来嵌套，以使绑定显式化或链式调用函数。
...
A2UI 协议旨在与大型语言模型以三步循环方式使用：

1.  **提示：** 为 LLM 构建一个提示，包括：
    - 要生成的期望 UI。
    - A2UI JSON 模式，包括组件目录。
    - 有效 A2UI JSON 的示例。
...
3.  **验证：** 根据 A2UI 模式验证生成的 JSON。如果 JSON 有效，可以将其发送给客户端进行渲染。如果无效，可以将错误在后续提示中报告给 LLM，使其能够自我纠正。

此循环允许高度的灵活性和鲁棒性，因为系统可以利用 LLM 的生成能力，同时仍然强制执行 UI 协议的结构完整性。

### 标准验证错误格式

如果验证失败，客户端（或代表客户端行事的系统）应向 LLM 发送 `error` 消息。为确保 LLM 能够理解并纠正错误，请在 `error` 消息载荷中使用以下标准格式：

- `code`（字符串，必需）：必须为 `"VALIDATION_FAILED"`。
- `surfaceId`（字符串，必需）：发生错误的 Surface 的 ID。
- `path`（字符串，必需）：指向验证失败字段的 JSON Pointer（例如 `/components/0/text`）。
- `message`（字符串，必需）：关于验证失败原因的简短一句话描述。
...
在 A2UI v0.10 中，能力和其他元数据通过**传输层元数据**或初始化载荷（例如，A2A 元数据、Agent Card 或 MCP 初始化）交换，而不是作为一等 A2UI 消息。

#### 服务器能力

服务器（或 Agent）使用 [`server_capabilities.json`] 模式通告其能力。这指示它可以为哪些目录生成 UI，以及是否接受客户端的内联目录。具体机制取决于传输层（例如，A2A AgentCard 中的 `params` 对象，或 MCP 中的服务器能力）。
...
- `supportedCatalogIds`（字符串数组，必需）：支持的目录的 URI。
- `inlineCatalogs`：由客户端直接提供的内联目录定义数组（适用于自定义或临时组件和函数）。
